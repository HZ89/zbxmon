...
class MySQL_Monitor(object):
    status_sql={'global status'  :{'sql':'SHOW /*!50002 GLOBAL */ STATUS',   #usage
                                  'fields':['Variable_name','Value'],
                                  },
          'variables'           :{'sql':'SHOW VARIABLES',  #usage
                                  'fields':['Variable_name','Value'],
                                  },
          'slave status'        :{'sql':'SHOW SLAVE STATUS', #REPLICATION CLIENT
                                  'cursor':MySQLdb.cursors.DictCursor,
                                  'dict':True
                                  },
          'master logs'         :{'sql':'SHOW MASTER LOGS',  #REPLICATION CLIENT
                                  'fields':['Log_name','File_size']
                                  },
          'processlist'         :{'sql':'SHOW PROCESSLIST',  #process
                                  'fields':['id','Info','db','State','Host','Command','User','Time']
                                  },
          'engines'             :{'sql':'SHOW ENGINES',   #usage
                                  'fields':['Engine','Comment','Transactions','Support','XA','Savepoints']
                                  },
          'innodb status'       :{'sql':'SHOW /*!50000 ENGINE*/ INNODB STATUS', #REPLICATION CLIENT
                                  'fields':['Type','Status','Name'],
                                  'split':['status']
                                  },
          'query time histogram':{'sql':'''SELECT `count`, ROUND(total * 1000000) AS total FROM INFORMATION_SCHEMA.QUERY_RESPONSE_TIME WHERE `time` <> 'TOO LONG' ''',
                                  },
         }    
    sections_struct=[
    ...
    def get_innodb_status(cls, text):
        ...
        for line in string.split('\n'):
            ...
            if line.startswith('Mutex spin waits'):
                ...
                row=[int(i) for i in re.findall('Mutex spin waits (\d+), rounds (\d+), OS waits (\d+)',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('RW-shared spins (\d+),(?: rounds (\d+),)? OS waits (\d+)',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('RW-excl spins (\d+),(?: rounds (\d+),)? OS waits (\d+)',line) if len(i)>0]
                if len(row)==3:  row.pop(1)
                status['spin_waits'].append(row[0])
                status['os_waits'].append(row[1]) 
                ...
                row=re.findall('for (\d+.\d+) seconds the semaphore:',line)
            ...
            #TRANSACTIONS
                ...
                row=[i for i in re.findall('Trx id counter(?: (\d+))? (\w+)',line) if len(i)>0]
                ...
                row=[i for i in re.findall('''Purge done for trx's n:o <(?: (\w+))? (\w+) undo n:o < 0 0''',line) if len(i)>0]
                ...
                if line.find('ACTIVE')>0: status['active_transactions']+=1
                ...
                row=re.findall('mysql tables in use (\d+), locked (\d+)',line)
                ...
                row=re.findall('(\d+) lock struct(s)',line)
            ...
            # FILE I/O
                ...
                row=re.findall('(\d+) OS file reads, (\d+) OS file writes, (\d+) OS fsyncs',line)
                ...
                row=re.findall('Pending normal aio reads: (\d+), aio writes: (\d+),',line)
                ...
                row=re.findall('''ibuf aio reads: (\d+), log i/o's: (\d+), sync i/o's: (\d+)''',line)
                ...
                row=re.findall('Pending flushes (fsync) log: (\d+); buffer pool: (\d+)')
            ...
            # INSERT BUFFER AND ADAPTIVE HASH INDEX
                ...
                row=re.findall('Ibuf for space 0: size (\d+), free list len (\d+), seg size (\d+)',line)
                ...
                row=[i for i in re.findall('Ibuf: size (\d+), free list len (\d+), seg size (\d+),(?: (\d+) merges)?',line) if len(i)>0]
                status['ibuf_used_cells']=int(row[0])
                status['ibuf_free_cells']=int(row[1])
                status['ibuf_cell_count']=int(row[2])
                if len(row)==4:
                    status['ibuf_merged']=int(row[3])
            elif line.find(', delete mark ')>0 and prev_line.startswith('merged operations:'):
                # Output of show engine innodb status has changed in 5.5
                # merged operations:
                # insert 593983, delete mark 387006, delete 73092  
                row=re.findall(' insert (\d+), delete mark (\d+), delete (\d+)',line)
                status['ibuf_inserts']=int(row[0])
                status['ibuf_merged']=int(row[0])+int(row[1])+int(row[2])
            elif line.find(' merged recs, ')>0:
                # 19817685 inserts, 19817684 merged recs, 3552620 merges
                row=[int(i) for i in re.findall('(d+) inserts, (\d+) merged recs, (\d+) merges',line)]
                ...
                row=[int(i) for i in re.findall('Hash table size (\d+),(?: used cells (\d+))?',line) if len(i)>0]
            ...
            # LOG
                ...
                row=[int(i) for i in re.findall('(\d+) pending log writes, (\d+) pending chkp writes',line)]
                ...
                row=[i for i in re.findall('Log sequence number (\w+)(?: (\w+))?',line) if len(i)>0]
                ...
                row=[i for i in re.findall('Log flushed up to   (\w+)(?: (\w+))?',line) if len(i)>0]
                ...
                row=[i for i in re.findall('Last checkpoint at  (\w+)(?: \w+)',line) if len(i)>0]
            ...
            # BUFFER POOL AND MEMORY
                ...
                row=[int(i) for i in re.findall('Total memory allocated (\d+)(?:; in additional pool allocated (\d+))?',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('Dictionary cache    145525560',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('Threads             (\d+)',line) if len(i)>0]
                status['thread_hash_memory']=row[0]
            elif line.startswith('innodb_io_pattern   '):
                #   innodb_io_pattern   0 	(0 + 0)
                row=[int(i) for i in re.findall('innodb_io_pattern   (\d+)',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('Pages read (\d+), created (\d+), written (\d+)',line) if len(i)>0]
            ...
            # ROW OPERATIONS
                ...
                row=[int(i) for i in re.findall('Number of rows inserted (\d+), updated (\d+), deleted (\d+), read (\d+)',line) if len(i)>0]
                ...
                row=[int(i) for i in re.findall('(\d+) queries inside InnoDB, (\d+) queries in queue',line) if len(i)>0]
    ...
    def get_data(cls,host=None,port=None,user=None,passwd=None):
        def _run_query(query,conn,cursor=MySQLdb.cursors.Cursor):
            try:
        ...
        try:
            ...
            res=_run_query("SHOW /*!50002 GLOBAL */ STATUS",conn)
            if res and len(res)>0:
                status.update(dict(res))
            res=_run_query("SHOW VARIABLES",conn)
            if res and len(res)>0:
                status.update(dict(res))
            res=_run_query("show slave status",conn,MySQLdb.cursors.DictCursor)
            if res and len(res)>0:
                status.update(res[0])
            ...
            res=_run_query("SHOW MASTER LOGS",conn)
            if res and len(res)>0:
                status['binary_log_space']=sum([i[1] for i in res])
            res=_run_query("SHOW PROCESSLIST",conn,MySQLdb.cursors.DictCursor)
            if res and len(res)>0:
                for row in res:
                    state=row['state']
            ...
            engines={}
            res=_run_query("SHOW ENGINES",conn)
            if res and len(res)>0:
                engines.update(dict([i[:2] for i in res]))
            if engines.has_key('InnoDB') and engines['InnoDB'] in ('YES','DEFAULT'):
                res=_run_query("SHOW /*!50000 ENGINE*/ INNODB STATUS",conn,MySQLdb.cursors.DictCursor)
                if res and len(res)>0:
            ...
            if (status.has_key('have_response_time_distribution') and status['have_response_time_distribution']=='YES') \
               or ((status.has_key('query_response_time_stats') and status['query_response_time_stats'])):
                res=_run_query("SELECT `count`, ROUND(total * 1000000) AS total FROM INFORMATION_SCHEMA.QUERY_RESPONSE_TIME WHERE `time` <> 'TOO LONG'",conn)
                if res and len(res)>0:
                    rn=len(res)
