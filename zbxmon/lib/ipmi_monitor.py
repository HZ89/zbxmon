...
def discovery_ipmi(arg=''):
    ...
    if not arg:
        ...
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool sdr type fan | grep -v 'Fully Redundant' | cut -d '|' -f 1 ")
        if status == 0:
    ...
    # cpu memory system io
        ...
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool sdr elist full | grep -E '(CPU|IO|MEM|SYS)'| cut -d '|' -f 1")
        if status == 0:
    ...
    # current voltage temp
        ...
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool sdr elist full | grep -E '(Inlet Temp|Exhaust Temp|Current|Voltage)'| cut -d '|' -f 1")
        if status == 0:
        ...
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool chassis status | grep -E '(System Power|Power Overload)' | cut -d ':' -f 1")
        if status == 0:
        ...
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool sel info | grep -v Information | cut -d ':' -f 1")
        if status == 0:
...
#    print instance_name
    ...
    if arg == 'VCT':
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool sdr elist full | grep -E '(Inlet Temp|Exhaust Temp|Current|Voltage)'")
        if status == 0:
            for vct in output.split('\n'):
                l_vct = vct.split('|')
    ...
    if arg == 'PW':
        (status, output) = commands.getstatusoutput("/usr/bin/ipmitool chassis status | grep -E '(System Power|Power Overload)'")    
        if status == 0:
            for pw in output.split('\n'):
                l_pw = pw.split(':')
