        ...
        try:
            ...
            if str(status.get('query_cache_type', 'OFF')).lower() == 'on':
                ...
                status['qcache_usage_rate'] = '{0:.2f}'.format(
                    float(status.get('query_cache_size', 0) - status.get('Qcache_free_memory', 0)) / (
                    0.001 + status.get(
            ...
            status['innodb_buffer_pool_hit_rate'] = '{0:.2f}'.format((1 - float(
                status.get('Innodb_buffer_pool_reads', 0)) / (0.001 + status.get('Innodb_buffer_pool_read_requests',
                                                                                 1))) * 100.0)
            # 表扫描使用索引比例
            ...
            if res and len(res) > 0:
                ...
                if slave_status.has_key('Slave_SQL_Running') or slave_status.has_key(
                        'Slave_IO_Running') or slave_status.has_key('Seconds_Behind_Master'):
                ...
                slave_status['Slave_SQL_Running'] = str(
                    slave_status.get('Slave_SQL_Running', 'NO')).lower() == 'yes' and 1 or 0
                slave_status['Slave_IO_Running'] = str(
                    slave_status.get('Slave_IO_Running', 'NO')).lower() == 'yes' and 1 or 0
                slave_status['slave_lag'] = int(str(slave_status.get('Seconds_Behind_Master', 0)).lower() in (
                'null', 'none') and "0" or slave_status.get('Seconds_Behind_Master', 0))
            ...
            # TPS QPS
            status['questions'] = status.get('Com_select', 0) + status.get('Com_update', 0) + status.get('Com_insert',
                                                                                                         0) + status.get(
                'Com_delete', 0)
            status['transactions'] = status.get('Com_update', 0) + status.get('Com_insert', 0) + status.get(
                'Com_delete', 0)
            # innodb dead locks
